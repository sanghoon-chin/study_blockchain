    ...
    private generateTxid() {
        ...
        if (this.type === 'tx') {
            ...
            (this.vin as ITxInput[]).forEach(txin => {
                IN += (little_endian(txin.txid) as string) + little_endian(String(txin.vout)) + Number(txin.scriptSig.length / 2).toString(16) + txin.scriptSig + 'FFFFFFFF';
            })
